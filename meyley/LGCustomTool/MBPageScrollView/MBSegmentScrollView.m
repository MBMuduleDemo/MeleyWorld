...
- (instancetype)initWithFrame:(CGRect)frame segmentStyle:(MBSegmentStyle *)style titles:(NSArray *)titles margin:(CGFloat)margin titleDidClick:(MBSegmentTitleClickBlock)block {
...
- (void)setupTitleLabels {
    for (NSInteger i = 0; i < self.titles.count; i++) {
        ...
        label.backgroundColor = self.segmentStyle.normalBackColor;
        label.cornerRidus = self.segmentStyle.titleCornerRadius;
...
- (void)setupTitleLabelPosition {
    ...
    for (NSInteger i = 0; i < self.titleLabels.count; i++) {
        ...
        label.frame = CGRectMake(titleX+self.titleMargin, 0, labelW, titleH);
    }
    ...
    if (firstLabel) {
        ...
        firstLabel.backgroundColor = self.segmentStyle.selectedBackColor;
    }
    ...
    [UIView animateWithDuration:duration animations:^{
        ...
        if (self.scrollLine) {
            ...
            if (self.segmentStyle.scrollLineWidth) {
                ...
                self.scrollLine.frame = CGRectMake(currentLabel.origin.x, self.frame.size.height - self.segmentStyle.scrollLineHeight -self.segmentStyle.scrollLineBottomMargin, currentLabel.frame.size.width, self.segmentStyle.scrollLineHeight);
            }
        }
        
        if (self.coverLayer) {
            
            self.coverLayer.frame = CGRectMake(currentLabel.origin.x - self.segmentStyle.coverHorizatalMargin, coverY, currentLabel.frame.size.width + 2*self.segmentStyle.coverHorizatalMargin, coverH);
        }
    ...
    if (!self.segmentStyle.isGradualChangeTitleColor) {
        for (NSInteger i = 0; i < self.titleLabels.count; i++) {
            ...
            if (i == currentIndex) {
                ...
                label.backgroundColor = self.segmentStyle.selectedBackColor;
            }else {
                label.textColor = self.segmentStyle.normalTitleColor;
                label.backgroundColor = self.segmentStyle.normalBackColor;
            }
...
- (void)adjustUIWithProgress:(CGFloat)progress lastIndex:(NSInteger)lastIndex currentIndex:(NSInteger)currentIndex {
    ...
    if (self.scrollLine) {
        if (self.segmentStyle.scrollLineWidth) {
            ...
            if (currentIndex>lastIndex) {   //向右滑动
                ...
                self.scrollLine.frame = CGRectMake(lastLabel.center.x-self.segmentStyle.scrollLineWidth/2+progress*xDistance, self.scrollLine.origin.y, self.segmentStyle.scrollLineWidth+wProgress*xDistance, self.scrollLine.frame.size.height);
            }else {     //向左滑动
            ...
            self.scrollLine.frame = CGRectMake(lastLabel.origin.x+progress*xDistance, self.scrollLine.origin.y, lastLabel.frame.size.width+progress*wDistance, self.scrollLine.frame.size.height);
        }
    }
    ...
    if (self.coverLayer) {
        CGFloat xDistance = currentLabel.frame.origin.x - lastLabel.frame.origin.x;
        CGFloat wDistance = currentLabel.frame.size.width - lastLabel.frame.size.width;
        self.coverLayer.frame = CGRectMake(lastLabel.origin.x-self.segmentStyle.coverHorizatalMargin+xDistance*progress, self.coverLayer.frame.origin.y, lastLabel.frame.size.width+wDistance*progress, self.coverLayer.frame.size.height);
    }
