//
//  LGMyServiceVipViewController.m
//  meyley
//
...
@interface LGMyServiceVipViewController ()<UITableViewDataSource,UITableViewDelegate>
...
@implementation LGMyServiceVipViewController
...
- (void)viewDidLoad {
    ...
    [self getMyServiceUserData];
}
- (void)getMyServiceUserData {
    ...
    [RequestUtil withGET:@"/api/popCenter/getMyServeUser.action" parameters:action success:^(NSURLSessionDataTask *task, id responseObject) {
        
        if ([[NSString stringWithFormat:@"%@",responseObject[@"status"]] isEqualToString:@"1"]) {
            NSMutableArray *tmp = [LGMyUserModel mj_objectArrayWithKeyValuesArray:responseObject[@"userOfWaiter"]];
...
- (UIView *)tableViewBackView {
    ...
    if (!_tableViewBackView) {
        ...
        tableViewBackLabel.text = @"你还没有服务的会员哟";
