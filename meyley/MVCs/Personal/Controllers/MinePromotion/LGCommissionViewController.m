...
@interface LGCommissionViewController ()<UITableViewDelegate,UITableViewDataSource>
...
- (void)viewDidLoad {
    ...
    self.navigationItem.title = @"服务佣金记录";
    
    self.dataArray = [NSMutableArray array];
    [self.view addSubview:self.tableView];
    
    [self getUserCommissionData];
}
- (void)getUserCommissionData {
    ...
    [RequestUtil withGET:@"/api/ecs/affiliate/list.action" parameters:action success:^(NSURLSessionDataTask *task, id responseObject) {
        
        if ([[NSString stringWithFormat:@"%@",responseObject[@"status"]] isEqualToString:@"1"]) {
            NSMutableArray *tmp = [LGMyCommissionModel mj_objectArrayWithKeyValuesArray:responseObject[@"result"]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    LGCommissionTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:NSStringFromClass([LGCommissionTableViewCell class]) forIndexPath:indexPath];
...
- (UITableView *)tableView {
    ...
    if (!_tableView) {
        ...
        [_tableView registerClass:[LGCommissionTableViewCell class] forCellReuseIdentifier:NSStringFromClass([LGCommissionTableViewCell class])];
    }
    return _tableView;
}
- (UIView *)tableViewBackView {
    ...
    if (!_tableViewBackView) {
        ...
        tableViewBackLabel.text = @"你还没有佣金记录哟";
