...
@property (nonatomic , strong)MBButton *filterBtn; //筛选
...
-(void)priceBtnTapAction:(UIButton *)sender{
    ...
    if (self.priceDesc == NO) {
        [self.priceBtn setImage:[UIImage imageNamed:@"selectDown"] forState:UIControlStateNormal];
    }else{
        [self.priceBtn setImage:[UIImage imageNamed:@"selectUp"] forState:UIControlStateNormal];
    }
...
- (void)FilterBtnTapAction:(MBButton *)filterBtn {
    if ([self.delegate respondsToSelector:@selector(filterBrandChildCategoryGoods)]) {
        [self.delegate filterBrandChildCategoryGoods];
    }
}
#pragma mark -- 创建控件
-(void)creatSubView{
...
-(MBButton *)priceBtn{
    if (!_priceBtn) {
        ...
        [_priceBtn setImage:[UIImage imageNamed:@"xiala"] forState:UIControlStateNormal];
...
-(MBButton *)filterBtn{
    if (!_filterBtn) {
        _filterBtn = [MBButton buttonWithType:UIButtonTypeCustom];
        [_filterBtn setTitle:@"筛选" forState:UIControlStateNormal];
        [_filterBtn setImage:[UIImage imageNamed:@"selNormal"] forState:UIControlStateNormal];
        [_filterBtn setTitleColor:RGB(97, 97, 97) forState:UIControlStateNormal];
        [_filterBtn setTitleColor:RGB(255, 0, 82) forState:UIControlStateSelected];
        _filterBtn.titleLabel.font = LGFont(13);
        _filterBtn.type = MBButtonTypeRightImageLeftTitle;
        [_filterBtn addTarget:self action:@selector(FilterBtnTapAction:) forControlEvents:UIControlEventTouchUpInside];
    }
    return _filterBtn;
}
