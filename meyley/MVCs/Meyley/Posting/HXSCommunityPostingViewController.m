...
static NSString *const textViewPlaceholdStr = @"此时此刻，想和大家分享什么？";
static CGFloat   const maxImageSizeM       = 200;   // 最大上传限制为300K
...
- (void)initTheTableView
{
    ...
    inputTextBgView.backgroundColor = [UIColor whiteColor];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (section == kPostingSection_Content) {
        if (row == 0) {
            ...
            [self.communityPostingPhotoCell setImageViewDeleteBlock:^(UIImageView *imageView)
             {
                [weakSelf deletePhoto:imageView];
             }];
            [self.communityPostingPhotoCell setPostArticleBlock:^{
                MLPostingArticleViewController *controller = [MLPostingArticleViewController controllerFromXib];
...
- (void)deletePhoto:(UIImageView *)imageView {
    ...
    if(tag < _photoTotalArray.count) {
        [_photoTotalArray removeObjectAtIndex:tag];
    }
    [self.tableView reloadData];
}
...
- (void)chooisePhotoes:(UIImageView *)imageView
{
    ...
    if (self.communityPostingPhotoCell.images.count < maxUploadPhotoNums) {
        if (tag == self.communityPostingPhotoCell.images.count ||
...
- (void)jumpToTakePhotoViewOrAlbumView:(BOOL)isCamera
{
    if(isCamera) {
        ...
        _pickerController.takePictureImageName = @"takePhoto";
...
- (void)postTheThreadNetworking
{
    ...
    [self.postModel postTheContent:_inputTextView.text photos:_photoTotalArray complete:^(HXSErrorCode code, NSString *message, NSString *postIdStr) {
