...
//    HXSCommunityCommentUser *communityCommentedUser = commentEntity.commentedUser;  //被评论的人
    ...
    if ([userId isEqualToString:[commentEntity.commentUserId stringValue]]) {
        [self.replyOrDeleteButton setImage:[UIImage imageNamed:@"mlw-xl"] forState:UIControlStateNormal];
   ...
   [self.userProfileImageView sd_setImageWithURL:[NSURL URLWithString:commentEntity.headPic] placeholderImage:[UIImage imageNamed:@"dsfdl-tx"]];
    
    self.userNickNameLabel.text = commentEntity.commentUserName;
    
    NSString *createTimeStr = [self updateTimeForRow:[commentEntity.createTime integerValue]/1000];
    
    self.replyTimeLabel.text = createTimeStr;
    
    [_replyContentLabel setText:commentEntity.content];
